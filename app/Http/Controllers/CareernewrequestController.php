...
use App\Models\Careernewrequest;
...
class CareernewrequestController extends Controller {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'careernewrequest';
	static $per_page	= '50';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Careernewrequest();	
		
		$this->info = $this->model->makeInfo( $this->module);	
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'careernewrequest',
	...
	function store( Request $request  )	{
	    ...
	    if($request->status == 7){
    	    ...
    	    $template='selected';
    	    $message='message';
            $data['date'] = date('M j, Y', strtotime($request->date));
            $data['time'] = date('h:s a', strtotime($request->date));
            $data['phone'] = $request->phone;
            $data['name'] = $request->add_info1;
            $data['email'] = $request->email;
            $data['job'] = Helper::get_job_by_id($request->job)->title;
	...
	public static function display(  )
	{
		...
		$model  = new Careernewrequest();
		$info = $model::makeInfo('careernewrequest');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('careernewrequest.public.view',$data);			
			}			
		} 
		else {
			...
			return view('careernewrequest.public.index',$data);	
		}
