...
class HomeController extends Controller {
	...
	public function index(Request $request){
		...
		if($page !='') {
			...
			//$blogs = \DB::table('tb_pages')->where('status','enable')->where('pagetype','post')->get();
			...
			//$csr = \DB::table('tb_pages')->where('cid', '3')->where('status','enable')->where('pagetype','post')->get();
			$csr_id = \DB::table('blog_category')->where('status', 1)->orderBy('id', 'desc')->where('title', 'LIKE', "%csr%")->first();
			$csr = \DB::table('blog')->where('status', 1)->where('category', $csr_id->id)->orderBy('id', 'desc')->paginate(6);
			foreach($csr as $data) {
				$data->category = Helper::get_blog_category_by_id($data->category);
				$data->user = Helper::get_user_by_id($data->add_info);
			}
			
			$events	= \DB::table('vmsl_news_events')->where('status', 1)->orderBy('date_time','desc')->get();
		 
            $this->data['our_partners'] = \DB::table('our_partner')->orderBy('serial_number', 'desc')->where('status', 1)->get();
			$this->data['blogs'] = $blogs; 
			$this->data['events'] = $events; 
			$this->data['achievements'] = \DB::table('vmsl_achievement')->orderBy('id', 'desc')->where('status', 1)->get();
			$this->data['csrs'] = $csr;
			$this->data['testimonials']	= \DB::table('vmsl_testimonial')->where('status',1)->orderBy('id','desc')->get();
			$this->data['website_setting']	= \DB::table('website_setting')->first();
			$this->data['counter']	= \DB::table('vmsl_counter')->first();
			...
			$this->data['teams'] = \DB::table('vmsl_management_team')->where('status','1')->where('type', 1)->orderBy('priority', 'DESC')->get();
            $this->data['loans'] = \DB::table('vmsl_loan')->where('status','1')->orderBy('priority', 'DESC')->limit('4')->get();
            $this->data['eventCategory']	= DB::table('vmsl_event_category')->where('status', 1)->orderBy('id', 'desc')->get();
			...
			$slider = \DB::table('vmsl_slider')->where('status','1')->orderBy('id', 'desc')->get();
			
			$type = \DB::table('vmsl_team_type')->where('name', 'LIKE', "%Board of Directors%")->first();
			
			
			$teams = $bod = \DB::table('vmsl_management_team')->where('type', $type->id)->orderBy('id', 'desc')->get();
	...
	public function csr( Request $request ,  $category = ''){
		$csr_id = \DB::table('blog_category')->where('status', 1)->where('title', 'LIKE', "%csr%")->first();
		$posts = \DB::table('blog')->where('status', 1)->where('category', $csr_id->id)->orderBy('id', 'desc')->get();
		...
		$this->data['posts']
		= $posts;
		$this->data['pages'] = 'layouts.'.config('sximo.cnf_theme').'.blog.csr';
		$page = 'layouts.'.config('sximo.cnf_theme').'.index';
		return view( $page , $this->data);	
	}
	...
	public function posts( Request $request ,  $category = ''){
		...
		$csr_id = \DB::table('blog_category')->where('status', 1)->where('title', 'LIKE', "%blog%")->first();
		$posts = \DB::table('blog')->where('status', 1)->where('category', $csr_id->id)->paginate(6);
	...
	public function read(Request $request, $read = ''){
		$row = \DB::table('blog')->where('updated_at', 'LIKE', "%".$read."%")->first();
		
		$category = \DB::table('blog_category')->where('id', $row->category)->first();
		
		$popular = \DB::table('blog')->where('category', $category->id)->where('status', 1)->OrderBy('id', 'DESC')->limit(6)->get();
		
		if($category->title == 'Financial Literacy'){
		    $latest_title = 'Latest Activities';
		}else{
		     $latest_title = 'Latest '.$category->title;
		}
