...
use App\Models\Imagegallerycontroller;
use Illuminate\Http\Request;
use Illuminate\Pagination\LengthAwarePaginator as Paginator;
use Validator, Input, Redirect ; 
use DB;

class ImagegallerycontrollerController extends Controller {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'imagegallerycontroller';
	static $per_page	= '50';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Imagegallerycontroller();	
		
		$this->info = $this->model->makeInfo( $this->module);	
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'imagegallerycontroller',
			...
			default:
				...
				if ($validator->passes()) 
				{
					...
					if($image_name){
    					...
    					DB::table('image_gallery')->where('id', $id)->update([
    					    'link' => $url.'/uploads/images/gallery/'.$image_name 
    					]);
					}
	...
	public static function display(  )
	{
		...
		$model  = new Imagegallerycontroller();
		$info = $model::makeInfo('imagegallerycontroller');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('imagegallerycontroller.public.view',$data);			
			}			
		} 
		else {
			...
			return view('imagegallerycontroller.public.index',$data);	
		}
