...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'bifflservices?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
	...
	<div class="col-md-12">
				...
				{!! Form::hidden('id', $row['id']) !!}					
						...
						<div class="image-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["image"],"/uploads/images/services") !!}
						</div>
						...
						<div class="banner-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["banner"],"/uploads/images/services") !!}
						</div>
					...
					<select name='status' rows='5' required  class='select2 '  > 
						...
						?></select> 
									  ...
									  </div> {!! Form::hidden('created_at', $row['created_at']) !!}{!! Form::hidden('updated_at', $row['updated_at']) !!}{!! Form::hidden('add_info2', $row['add_info2']) !!}</fieldset></div>
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("bifflservices/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
