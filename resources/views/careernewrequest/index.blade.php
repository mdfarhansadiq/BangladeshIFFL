...
<div class="toolbar-nav" >   
	<div class="row">
		<div class="col-md-8"> 	
			@if($access['is_add'] ==1)
			<a href="{{ url('careernewrequest/create?return='.$return) }}" class="btn btn-default btn-sm"  
			...
			<div class="btn-group">
		        ...
		        <ul class="dropdown-menu" id="mycustommenu">
				...
				<div class="dropdown-divider"></div>
					<li class="nav-item" style="padding: 7px;">
					    ...
					    <input type="datetime-local" class="form-control" id="calldate" style="width:100%; padding:3px;">
					 </li>
				@php
				    use DB;
				    $statuses = DB::table('vmsl_status')->get();
				@endphp
				@foreach($statuses as $status)
				    <div class="dropdown-divider"></div>
				    <li class="nav-item"><a href="javascript://ajax" class="callfor nav-link" data-callfor="{{$status->id}}">{{  $status->name }}</a></li>
				@endforeach
		        </ul>
		    </div>    
		</div>
...
<div class="table-container">
			...
			<!-- Table Grid -->
 			...
 			{!! Form::open(array('url'=>'careernewrequest?'.$return, 'class'=>'form-horizontal m-t' ,'id' =>'SximoTable' )) !!}
							...
							<td>
							 	...
							 	<div class="dropdown">
								  ...
								  <ul class="dropdown-menu">
									...
									<li class="nav-item"><a href="{{ url('careernewrequest/'.$row->id.'?return='.$return)}}" class="nav-link tips" title="{{ __('core.btn_view') }}"> {{ __('core.btn_view') }} </a></li>
									@endif
									@if($access['is_edit'] ==1)
									<li class="nav-item"><a  href="{{ url('careernewrequest/'.$row->id.'/edit?return='.$return) }}" class="nav-link  tips" title="{{ __('core.btn_edit') }}"> {{ __('core.btn_edit') }} </a></li>
									@endif
									<div class="dropdown-divider"></div>
									@if($access['is_remove'] ==1)
										<li class="nav-item"><a href="javascript://ajax"  onclick="SximoDelete();" class="nav-link  tips" title="{{ __('core.btn_remove') }}">
...
$(document).ready(function(){
	...
	$('.callfor').click(function() {
	    var calldate = $('#calldate').val();
	    var call_id = $(this).attr('data-callfor');
        const final = [];
        $('.ids:checked').each(function(){        
          ...
          if(final.length > 0){
              if(!calldate){
                    ...
                    $.ajax({
                      type:'POST',
                      url:"{{ route('all-call-for') }}",
                      data: {"call_id":call_id, "ids":final, "calldate":calldate},
              ...
              alert('Please select minimum one candidate.');
          }
