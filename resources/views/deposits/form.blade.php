...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'deposits?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
	...
	<div class="col-md-12">
				...
				{!! Form::hidden('id', $row['id']) !!}					
					...
					$show_home_page_opt = array( '1' => 'YES' , ); ?>
					<select name='show_home_page' rows='5'   class='select2 '  > 
						<?php 
						foreach($show_home_page_opt as $key=>$val)
						{
							echo "<option  value ='$key' ".($row['show_home_page'] == $key ? " selected='selected' " : '' ).">$val</option>"; 						
						}						
						?></select> 
										 </div> 
										 <div class="col-md-2">
						...
						<div class="image-preview preview-upload">
						 ...
						 >{{ $row['details'] }}</textarea> 
									  ...
									  <div class="form-group row  " >
										<label for="Feature" class=" control-label col-md-4 text-left"> Feature </label>
										<div class="col-md-6">
						 ...
						 >{{ $row['feature'] }}</textarea> 
						     ...
						     class='form-control form-control-sm ' /> 
									  ...
									  <div class="form-group row  " >
										<label for="Eligibility" class=" control-label col-md-4 text-left"> Eligibility </label>
										<div class="col-md-6">
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#category").jCombo("{!! url('deposits/comboselect?filter=vmsl_deposit_category:id:name') !!}",
		{  selected_value : '{{ $row["category"] }}' });
		
		$("#status").jCombo("{!! url('deposits/comboselect?filter=vmsl_status:id:name') !!}",
		{  selected_value : '{{ $row["status"] }}' });
		 	
		 	 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("deposits/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
