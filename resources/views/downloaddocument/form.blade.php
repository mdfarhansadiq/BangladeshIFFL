...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'downloaddocument?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
	...
	<div class="col-md-12">
				...
				{!! Form::hidden('id', $row['id']) !!}					
						...
						<div class="document-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["document"],"/uploads/download") !!}
						</div>
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#category").jCombo("{!! url('downloaddocument/comboselect?filter=vsml_download_category:id:title') !!}",
		{  selected_value : '{{ $row["category"] }}' });
		 	
		 	 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("downloaddocument/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
