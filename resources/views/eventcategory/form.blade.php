...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'eventcategory?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
	...
	<div class="p-5">
		...
		<div class="row">
	<div class="col-md-12">
						<fieldset><legend> Event Category</legend>
				{!! Form::hidden('id', $row['id']) !!}					
					...
					<select name='status' rows='5' required  class='select2 '  > 
						...
						?></select> 
									  ...
									  </div> {!! Form::hidden('created_at', $row['created_at']) !!}{!! Form::hidden('updated_at', $row['updated_at']) !!}{!! Form::hidden('add1', $row['add1']) !!}{!! Form::hidden('add2', $row['add2']) !!}</fieldset></div>
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("eventcategory/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
