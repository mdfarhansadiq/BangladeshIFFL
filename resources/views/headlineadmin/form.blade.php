...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'headlineadmin?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
	...
	<div class="p-5">
		...
		<div class="row">
	<div class="col-md-12">
						<fieldset><legend> Headline</legend>
				{!! Form::hidden('id', $row['id']) !!}					
						...
						required     class='form-control form-control-sm ' /> 
				           ...
				           >{{ $row['description'] }}</textarea> 
									  ...
									  </div> {!! Form::hidden('slug', $row['slug']) !!}{!! Form::hidden('category', $row['category']) !!}					
									  <div class="form-group row  " >
					...
					<select name='status' rows='5'   class='select2 '  > 
						...
						?></select> 
									  ...
									  </div> {!! Form::hidden('add_info1', $row['add_info1']) !!}{!! Form::hidden('created_at', $row['created_at']) !!}</fieldset></div>
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("headlineadmin/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
