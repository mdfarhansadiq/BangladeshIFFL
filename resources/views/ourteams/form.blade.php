...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'ourteams?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
				...
				{!! Form::hidden('id', $row['id']) !!}					
						...
						required     class='form-control form-control-sm ' /> 
						     ...
						     class='form-control form-control-sm ' /> 
									  ...
									  </div> {!! Form::hidden('representing', $row['representing']) !!}					
									  <div class="form-group row  " >
						...
						<div class="image-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["image"],"/uploads/images/team") !!}
									  ...
									  </div> {!! Form::hidden('facebook', $row['facebook']) !!}{!! Form::hidden('twitter', $row['twitter']) !!}{!! Form::hidden('linkedin', $row['linkedin']) !!}{!! Form::hidden('instagram', $row['instagram']) !!}{!! Form::hidden('pinterest', $row['pinterest']) !!}{!! Form::hidden('whatsup', $row['whatsup']) !!}{!! Form::hidden('email', $row['email']) !!}{!! Form::hidden('dribbble', $row['dribbble']) !!}{!! Form::hidden('youtube', $row['youtube']) !!}					
									  <div class="form-group row  " >
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#type").jCombo("{!! url('ourteams/comboselect?filter=vmsl_team_type:id:name') !!}",
		{  selected_value : '{{ $row["type"] }}' });
		 	
		 	 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("ourteams/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
