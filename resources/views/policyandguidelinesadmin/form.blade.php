...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'policyandguidelinesadmin?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
	...
	<div class="p-5">
		...
		<div class="row">
	<div class="col-md-12">
						<fieldset><legend> Policy and Guidelines</legend>
				{!! Form::hidden('id', $row['id']) !!}					
						...
						<div class="document-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["document"],"/uploads/images/guidelines") !!}
						</div>
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#category").jCombo("{!! url('policyandguidelinesadmin/comboselect?filter=vsml_guidelines_category:id:title') !!}",
		{  selected_value : '{{ $row["category"] }}' });
		 	
		 	 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("policyandguidelinesadmin/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
