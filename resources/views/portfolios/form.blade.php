...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'portfolios?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
				...
				{!! Form::hidden('id', $row['id']) !!}					
						...
						<div class="image-preview preview-upload">
				           ...
				           >{{ $row['description'] }}</textarea> 
									  ...
									  </div> {!! Form::hidden('gallery', $row['gallery']) !!}					
									  <div class="form-group row  " >
					...
					<select name='status' rows='5' required  class='select2 '  > 
						...
						?></select> 
									  ...
									  </div> {!! Form::hidden('add1', $row['add1']) !!}{!! Form::hidden('created_at', $row['created_at']) !!}{!! Form::hidden('updated_at', $row['updated_at']) !!}</fieldset></div>
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#category").jCombo("{!! url('portfolios/comboselect?filter=vmsl_portfolio_category:id:title') !!}",
		{  selected_value : '{{ $row["category"] }}' });
		 	
		 	 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("portfolios/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
